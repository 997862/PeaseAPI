  ...
  <div class="flex min-h-screen items-center justify-center bg-gradient-to-br from-primary-50 to-indigo-100 px-4 py-12 sm:px-6 lg:px-8">
    <div class="w-full max-w-md">
      ...
      <div class="rounded-2xl bg-white p-8 shadow-xl">
        <h2 class="text-center text-2xl font-bold text-gray-900 mb-6">登录 PeaseAI</h2>
        ...
        <form class="space-y-5" @submit.prevent="handleLogin">
          ...
          <div>
            ...
            <input id="password" v-model="form.password" :type="showPassword ? 'text' : 'password'" required autocomplete="current-password" class="block w-full rounded-lg border border-gray-300 px-4 py-2.5 text-gray-900 placeholder-gray-400 shadow-sm focus:border-primary-500 focus:ring-2 focus:ring-primary-500/20 focus:outline-none transition" placeholder="请输入密码" />
          </div>
        ...
        <div class="mt-6 text-center text-sm text-gray-600">
          还没有账号？<router-link to="/register" class="font-medium text-primary-600 hover:text-primary-500">立即注册</router-link>
        </div>
      </div>
